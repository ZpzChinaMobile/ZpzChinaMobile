...
+(void)opensql{
    ...
    if (sqlite3_open([database_path UTF8String], &zpzchinaMobileDB)==SQLITE_OK) {
        NSLog(@"打开数据库成功!");
        NSString *createSQL = @"CREATE TABLE IF NOT EXISTS User (userToken Text ,userID Text,isFaceRegisted Text,faceCount Text,department Text,company Text,city Text,leaderLevel Text,office Text,province Text,region Text,street Text,supervisor Text); ";
...
+(void)InsertData:(NSDictionary *)dic{
    ...
    SqliteHelper *sqlite = [[SqliteHelper alloc] init];
	if ([sqlite open:DataBaseName]) {
        [sqlite executeQuery:@"INSERT INTO User(userToken,userID,isFaceRegisted,faceCount,department,company,city,leaderLevel,office,province,region ,street,supervisor) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?);",
         dic[@"userToken"],dic[@"userID"],dic[@"isFaceRegisted"],dic[@"faceCount"],dic[@"department"],dic[@"company"],dic[@"city"],dic[@"leaderLevel"],dic[@"office"],dic[@"province"],dic[@"region"],dic[@"street"],dic[@"supervisor"]];
	}
}
